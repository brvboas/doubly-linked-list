...
Node::Node(Node_entry item, Node *add_on, Node *pre) {
    entry = item;
    next = add_on;
    prev=pre;
}
...
bool LinkedList::insertFront(Node_entry el){
  Node *newNode;
  newNode = new Node(el);
...
bool LinkedList::insertRear(Node_entry el){
	Node *newNode;
	newNode=new Node(el);
	if(newNode == NULL) return false;
	if(front==NULL){
...
bool LinkedList::insert(Node_entry el){
  Node *newNode, *prevNode, *currentNode;//, *nextNode;
  newNode = new Node(el);
  if(newNode == NULL) return false;
  prevNode = NULL;
  currentNode = front;
